...
- Added a distinct Start Menu Organizer icon with transparent PNG and multi-size ICO assets.
- Restyled the app around the new midnight, cyan, mint, and amber identity. The Windows title bar now follows the dark interface.
- Added polished checkbox and scrollbar controls, clearer selection labels, shorter access status text, and quieter cleanup buttons.
- Added a safe demo workspace with realistic sample shortcuts for product tours and repeatable screenshots.
- Made reviewable operation plans the required path for cleanup, organization, and batch rename actions.
- Removed confirmation prompts and keyboard shortcut bindings. Actions now report through the status bar, activity log, and recovery journal.
- Fixed footer counts when a scan contains exactly one broken, junk, duplicate, or folder item.
- Added branded install shortcuts, install markers, guarded uninstall behavior, and a shortcut-free validation option.
- Replaced the release builder's missing `Get-FileHash` dependency with a .NET SHA256 implementation.
- Removed WinGet manifest authoring while keeping Scoop and Chocolatey metadata.
- Rebuilt the README around verified product screenshots, a shorter install path, safety details, and demo instructions.
...
- Administrator relaunch button with capability status display.
...
- Fixed the administrator relaunch button using the wrong script path when invoked from non-file scriptblocks.
...
- Fixed the administrator relaunch control missing from tab order.
...
- Kept synchronous plan execution available for the local regression test suite.
